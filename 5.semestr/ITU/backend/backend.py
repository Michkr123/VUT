...
def update_review(event_id, review_id):
    ...
    reviews[review_index]['date_edit'] = datetime.now().isoformat()
    ...
    reviews[review_index]['edited'] = True
    reviews[review_index]['last_edited'] = datetime.now().isoformat()
...
def delete_event(event_id):
    ...
    reviews = load_reviews()
    updated_reviews = [r for r in reviews if r['event_id'] != event_id]
    save_reviews(updated_reviews)
    
    # Remove event from all user profiles
    profiles = load_profiles()
    for profile in profiles:
        if event_id in profile.get('visitorActions', []):
            profile['visitorActions'].remove(event_id)
        if event_id in profile.get('workerActions', []):
            profile['workerActions'].remove(event_id)
    save_profiles(profiles)
    
    return jsonify(deleted_event), 200
...
def register_for_event(event_id):
    try:
        ...
        role = data.get('role')  # 'visitor' or 'worker'
        
        if not login or not role or role not in ['visitor', 'worker']:
            return jsonify({"error": "Invalid request data"}), 400

        profiles = load_profiles()
        profile_index = next((index for index, p in enumerate(profiles) if p['login'] == login), None)
        
        if profile_index is None:
            return jsonify({"error": "Profile not found"}), 404

        if role == 'visitor':
            if event_id not in profiles[profile_index]['visitorActions']:
                profiles[profile_index]['visitorActions'].append(event_id)
        else:
            if event_id not in profiles[profile_index]['workerActions']:
                profiles[profile_index]['workerActions'].append(event_id)
...
def unregister_from_event(event_id):
    try:
        ...
        if role == 'visitor':
            if event_id in profiles[profile_index]['visitorActions']:
                profiles[profile_index]['visitorActions'].remove(event_id)
        else:
            if event_id in profiles[profile_index]['workerActions']:
                profiles[profile_index]['workerActions'].remove(event_id)
