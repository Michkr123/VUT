...
void Stack_Init( Stack *stack ) {
	if (stack == NULL) {
		...
		stack->array = (char *)malloc(sizeof(char) * STACK_SIZE); // alokujememe pamet pro zasobnik o velikosti STACK_SIZE
		if (stack->array == NULL) { // osetreni neuspesne alokace pameti
			Stack_Error(SERR_INIT);
		}
		stack->topIndex = -1;
	}
...
bool Stack_IsEmpty( const Stack *stack ) {
	return (stack->topIndex == -1) ? true : false; // tercialni operator cond ? true : false
}
...
bool Stack_IsFull( const Stack *stack ) {
	return (stack->topIndex == STACK_SIZE - 1) ? true : false; // opet tercialni operator
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	if (Stack_IsEmpty(stack)) {
		...
		*dataPtr = stack->array[stack->topIndex]; // vracime ukazatel na vrchol zasobniku pomoci ukazatele
	}
}
...
void Stack_Pop( Stack *stack ) {
	if (!Stack_IsEmpty(stack)) {
		stack->array[stack->topIndex--] = '\0'; // vlozime misto znaku na vrcholu zasobniku '\0' a posunu ukazatel na jeho vrchol
	}
}
...
void Stack_Push( Stack *stack, char data ) {
	if (Stack_IsFull(stack)) { // osetreni vlozeni prvku do plneho zasobniku
		Stack_Error(SERR_PUSH);
	}
	else {
		stack->array[++stack->topIndex] = data; // nejdrive zvysime ukazatel na vrchol zasobniku a tepre pak zapiseme hodotu
	}
}
...
void Stack_Dispose( Stack *stack ) {
	free(stack->array); // uvolni pamet, ktera byla alokovana funkci Stack_Init
	stack->array = NULL;
}
