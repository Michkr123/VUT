...
V~této práci půjde právě o oklamání automatizované autentizace pomocí replikace vlastnoručního podpisu.
...
Statické charakteristiky jsou charakteristiky, které lze extrahovat z hotového podpisu.
Oproti tomu dynamické charakteristiky musí být zaznamenávány v průběhu podpisu.
...
K zaznamenání těchto dynamických vlastností je avšak zapotřebí tablet a speciální aktivní pero, které při psaní ukládá relevantní parametry.~\cite{DrahanskýMartin2011}  % parafráze Biometrie (str 233)
...
V~rámci této práce se zaměříme na senzor, jenž je první komponentu biometrického systému (obrázek níže).   % V
Tomuto senzoru budeme předkládat falešné biometriky, tedy v~našem případě falešného podpisu.~\cite{DrahanskýMartin2011}% parafráze Biometrie (str 78)
...
Navíc v dnešní moderní době, při existenci skenovacích a kopírovacích zařízení, je statický podpis náchylný na falzifikaci.~\cite{RakRoman2008}% Parafráze Biografie a identita člověka (str 440)
...
může být provedeno porovnáním významových bodů, klasifikátorem sousedů nebo neuronovou sítí.\cite{RakRoman2008}% Parafráze Biometrie a identita člověka (strana 441 442 443) - nebo https://theses.cz/id/odwy80/BP_Vchov.pdf
...
Falzifikát lze vytvořit pomocí stroje, je k tomu ale potřeba nasnímat pohyb ruky podepisujícího.~\cite{VaccaJohnR2007}% parafráze Biometric verification (str. 170)
...
Například NIST, působící na poli biometrických databází, TCSEC, Common Criteria, obojí zabývající se biometrií v rámci bezpečnosti.~\cite{DrahanskýMartin2011}% Biometrie str. 68
...
Obecně k zaznamenání podpisu lze také využít tablet s digitálním perem či mobilní telefon.
...
Následně stejná skupina provedla experiment kde subjektům umožnili studovat průběh podpisu.
...
Na závěr tedy můžeme říct, že nejdůležitější však je, že rizika spojená s použitím dynamického biometrického podpisu v různých situacích lze dobře 
zvládnout volbou správného nastavení hranice stupně shody pro akceptaci podpisu.~\cite{8585636} % https://ieeexplore-ieee-org.ezproxy.lib.vutbr.cz/stamp/stamp.jsp?tp=&arnumber=8585636
...
Z toho vyplývá, že například stres či jiné rozrušení může ovlivnit podpis tak, že dojde k jeho odmítnutí.
Dále může dojít k odmítnutí u osob, které mají nekonzistentní podpis nebo u jedinců, kterým nějaká nemoc či úraz ovlivnil písmo.
...
Návrh snímacího pera (obrázek) je navrhnut tak, aby fungoval jako normální pero, 
přičemž bylo zároveň možné snímat jeho pohyb a další dynamické vlastnosti.
...
Naprogramovat kód mikrokontroléru tak, aby co nejčastěji odesílal snímaná data přes USB kabel do počítače, kde se budou data ukládat.
